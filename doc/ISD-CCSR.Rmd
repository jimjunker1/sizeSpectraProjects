...
NM_nwt_pred = dw_dat_summ %>% 
  ...
  mutate(pred = NM_nwt_int + lmbar_n*-1)
...
NM_bwt_pred = dw_dat_summ %>% 
  ...
  mutate(pred = NM_bwt_int + lmbar_b*-1)
...
data_grid(lmbar_n = seq_range(lmbar_n, n = 51)) %>%
  ...
  ggplot()+
  geom_line(data = NM_nwt_pred, aes(x = lmbar_n, y = pred), linewidth = 1.1, linetype = 'dotted', color = 'darkgrey')+
  ...
  scale_y_continuous(name = expression(log[10]*"(N)"), limits = c(3, 7), expand = c(0,0), breaks = seq.int(3,7, by = 2), labels = format(10^seq.int(3,7, by = 2), scientific = TRUE))+
...
data_grid(lmbar_b = seq_range(lmbar_b, n = 51)) %>%
  ...
  ggplot()+
  geom_line(data = NM_bwt_pred, aes(x = lmbar_b, y = pred), linewidth = 1.1, linetype =   'dotted', color = 'darkgrey')+
