...
gridExtra::grid.arrange(isd_NMnwt_plot, isd_NMbwt_plot, ncol =2)

```

```{r residual from theory}
...
isd_NMnwt_plot = NM_nwt_pred %>% 
  add_epred_draws(NMnwt_res_theo, ndraws = 100) %>% 
  ggplot()+
  geom_line(aes(x = lambda_med, y = .epred, group = .draw), alpha = 0.1)+
  geom_point(aes(x = lambda_med, y = resid_theory, color = site_id))+
  scale_color_viridis(discrete = TRUE)+
  labs(subtitle = expression("N-"*M['abun-weighted']))+
  theme(legend.position = 'none')
...
isd_NMbwt_plot = NM_bwt_pred %>% 
  add_epred_draws(NMbwt_res_theo, ndraws = 100) %>% 
  ggplot()+
  geom_line(aes(x = lambda_med, y = .epred, group = .draw), alpha = 0.1)+
...
effectDf = NMnwt_res %>% 
  ...
  bind_rows(NMbwt_res %>% spread_draws(b_lambda_med) %>% mutate(model = 'NMbwt_res')) %>% 
  bind_rows(NMnwt_res_theo %>% spread_draws(b_lambda_med) %>% mutate(model = 'NMnwt_res_theo')) %>% 
  bind_rows(NMbwt_res_theo%>% spread_draws(b_lambda_med) %>% mutate(model = 'NMbwt_res_theo'))
