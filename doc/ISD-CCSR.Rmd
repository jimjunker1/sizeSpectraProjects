...
output = capture.output(brm(bf(ln_tot_s~lmbar_n0.75_s),
              data = dw_dat_summ,
...
NM_nwt_pred = dw_dat_summ %>% 
  ...
  mutate(pred = NM_nwt_int + lmbar_n0.75_s*-1)%>% 
  left_join(dw_dat_summ %>% select(site_id, lmbar_n0.75_s, ln_tot_s, lambda_med, log_gpp_s, log_om_s), by = 'lmbar_n0.75_s') %>% 
...
```{r N-Mn plot, fig.cap='Figure 1. The scaling of community abundances by community abundance-weighted body size across the NEON streams. The blue line represents a generalized additive model fit to the data. Transparent grey lines represent a random sample of posterior draws from a linear model (n = 100).'}
...
output = capture.output(brm(bf(NMnwt_res~lambda_med), 
                            data = dw_dat_summ,
                            family = gaussian(link = 'identity'),
                            iter = 1000,
                            thin = 1,
...
isd_NMnwt_plot = dw_dat_summ %>% 
  add_epred_draws(NMnwt_res, ndraws = 100) %>% 
  ggplot()+
...
```{r residual from theory, fig.cap = "Figure X. The relationship between the individual size distribution rate parameter $\\lambda$ and residuals from the theoretical cross community size relationship. The size of each point is scaled to the mean body size of the community."}
...
isd_NMnwt_plot = NM_nwt_pred %>% 
  ...
  ggplot()+
  geom_line(aes(x = lambda_med, y = .epred, group = .draw), alpha = 0.1)+
  geom_point(data = NM_nwt_pred, aes(x = lambda_med, y = resid_theory,  fill = site_id, size = lmbar_n0.75_s),color = 'black', shape = 21, alpha = 0.4)+
