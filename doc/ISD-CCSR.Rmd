...
output = capture.output(brm(bf(ln_tot_s~lmbar_n0.75_s+(1 + lmbar_n0.75_s|site_id)),
              data = dw_dat_summ,
...
data_grid(lmbar_n0.75_s = seq_range(lmbar_n0.75_s, n = 51)) %>%
  ...
  ggplot()+
  geom_line(data = NM_nwt_pred, aes(x = lmbar_n0.75_s, y = pred), linewidth = 1.1, linetype = 'dotted', color = 'darkgrey')+
  # geom_smooth(data = dw_dat_summ, aes(x = lmbar_n0.75_s, y = ln_tot_s, group = site_id, color = site_id, fill = site_id), se = FALSE, method = 'lm', alpha = 0.5)+
  geom_point(data = dw_dat_summ, aes(x = lmbar_n0.75_s, y = ln_tot_s, fill = site_id), shape = 21)+
  geom_line(aes(x = lmbar_n0.75_s, y = .epred, group = .draw), alpha = 0.1)+
  annotation_custom(as_grob(inset), xmin = 1.2, xmax = 4.5, ymin = 0.9, ymax = 3.5)+
